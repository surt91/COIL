...
const TIPS: { id: string; when(f: Fight): boolean; text: string }[] = [
  ...
  { id: 'lock', when: (f) => f.enemies.some((e) => e.intent.t === 'lock' && !e.intent.sever), text: 'A red dashed line: an enemy has latched onto a segment and will bite it after your move — if that segment is still within 1 tile of it (diagonals count). Bite the attacker to interrupt it, or move so the segment slides out of reach.' },
  ...
  { id: 'coil', when: (f) => computeCoils(f).some((c) => c.tiles.some((t) => f.enemies.some((e) => eq(e.pos, t)))), text: 'Coiled! Enemies enclosed by your body (walls help) are held in place and crushed every turn. The tighter the coil, the harder the crush: 1 tile = 3 damage, 2–3 = 2, 4–8 = 1.' },
