...
const TIPS: { id: string; when(f: Fight): boolean; text: string; urgent?: boolean }[] = [
  ...
  { id: 'hand', when: (f) => f.turn >= 2 && ops.hand(f).length > 0, text: 'The numbered items behind your head are your hand (1–3). Spend freely: they all come back next room.' },
  ...
  { id: 'coil', when: (f) => coiledEnemies(f).size > 0, text: 'Coiled! It can’t act, and takes the violet number every turn. Tighter coils crush harder.' },
