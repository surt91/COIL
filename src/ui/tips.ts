...
const TIPS: { id: string; when(f: Fight): boolean; text: string }[] = [
  ...
  { id: 'lock', when: (f) => f.enemies.some((e) => e.intent.t === 'lock' && !e.intent.sever), text: 'A red reticle: an enemy has latched onto that segment and will bite it after your move — if the segment is still inside the faint red box around the enemy (1 tile, diagonals count). Bite the attacker to knock it back and interrupt it (not if it is pinned against something; bosses only while wrapped or coiled), or move so the segment slides out of reach. A bite on your head is worse: it destroys the two segments right behind it — your hand.' },
