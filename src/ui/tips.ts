...
const TIPS: { id: string; when(f: Fight): boolean; text: string }[] = [
  ...
  { id: 'hand', when: (f) => f.turn >= 2 && ops.hand(f).length > 0, text: 'The glowing items right behind your head are your hand. Press 1–3 to play one. Playing an item consumes its segment — you get shorter.' },
  { id: 'lock', when: (f) => f.enemies.some((e) => e.intent.t === 'lock' && !e.intent.sever), text: 'A red reticle: an enemy has latched onto that segment and will bite it after your move — if the segment is still inside the faint red box around the enemy (1 tile, diagonals count). Bite the attacker to interrupt it, or move so the segment slides out of reach.' },
  ...
  { id: 'coil', when: (f) => computeCoils(f).some((c) => c.tiles.some((t) => f.enemies.some((e) => eq(e.pos, t)))), text: 'Coiled! Enemies enclosed by your body (walls help) are held in place and crushed every turn. The tighter the coil, the harder the crush: 1 tile = 3 damage, 2–3 = 2, 4–8 = 1, 9–12 = held only. You must keep moving — to keep a coil closed, chase your own tail.' },
  { id: 'wrap', when: (f) => f.enemies.some((e) => f.snake.body.filter((b) => Math.max(Math.abs(b.x - e.pos.x), Math.abs(b.y - e.pos.y)) === 1).length >= 2), text: 'The violet arcs around an enemy count how many of your tiles touch it (diagonals count). At 4 it is wrapped and squeezed for 1 damage every turn — even without a closed coil.' },
  ...
  { id: 'cleared', when: (f) => f.cleared && f.status === 'play', text: 'Room cleared! The exits are open. Remaining flesh carries over to the next room — it is your health and your currency, so grab food on the way out.' },
